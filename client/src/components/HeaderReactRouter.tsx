...
export function HeaderReactRouter() {
    ...
    <header className="bg-charcoal text-white">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-20">
          {/* Logo */}
          <Link to="/" className="text-2xl font-heading hover:text-sky-400 transition-colors">
            ...
            {navigation.map((item) => (
              <Link
                ...
                className={`font-semibold hover:text-sky-400 transition-colors ${
                  location.pathname.startsWith(item.href) ? 'text-sky-400' : ''
                }`}
              >
                {item.name}
              </Link>
