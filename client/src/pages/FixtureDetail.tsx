      ...
      <>
        ...
        <main className="py-16 bg-white min-h-screen">
          <div className="container mx-auto px-4 text-center">
            ...
            <Link to="/fixtures" className="text-sky-600 hover:text-sky-700 font-semibold">
              Return to Fixtures
            </Link>
          </div>
        </main>
    ...
    <>
      ...
      <main className="py-16 bg-slate-50 min-h-screen">
        <div className="container mx-auto px-4 max-w-4xl">
          <Link to="/fixtures" className="inline-flex items-center gap-2 text-charcoal hover:text-sky-600 mb-8 font-semibold transition-colors">
          ...
          <div className="bg-white border border-slate-200 p-8 mb-6">
            <div className="text-center mb-8">
              <span className="text-xs uppercase tracking-wider text-sky-600 bg-sky-50 px-4 py-2 font-semibold">
                {fixture.competition}
              </span>
              {fixture.status === 'completed' && (
                <span className="text-xs uppercase tracking-wider bg-sky-50 text-sky-700 px-4 py-2 ml-2 font-semibold">
                  Full Time
                </span>
              )}
              {fixture.status === 'upcoming' && (
                <span className="text-xs uppercase tracking-wider bg-blue-50 text-blue-700 px-4 py-2 ml-2 font-semibold">
                  Upcoming
                </span>
              )}
            </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {fixture.homeLineup.map((player, index) => (
                      <div key={index} className="flex items-center gap-3 bg-slate-50 p-3">
                        <div className="w-10 h-10 bg-charcoal text-white rounded-full flex items-center justify-center font-semibold">
                          {player.number}
                        </div>
                        <div>
          ...
          {fixture.status === 'upcoming' && (
            <div className="bg-white border border-slate-200 p-8">
              ...
              <div className="flex flex-wrap gap-4">
                <button className="bg-sky-600 hover:bg-sky-700 text-white px-8 py-3 transition-colors font-semibold">
                  Get Tickets
                </button>
                <button className="border-2 border-charcoal text-charcoal hover:bg-charcoal hover:text-white px-8 py-3 transition-colors font-semibold">
                  Set Reminder
                </button>
              </div>
            </div>
